...
async function main() {
  ...
  const txt = await waveFormsNFT.togglePublicSaleLive();
  console.log("togglePublicSaleLive txt.hash = ",txt.hash);
  const txtReceipt = await txt.wait();
  console.log("togglePublicSaleLive txt.hash = ",txtReceipt);
